...
export default function ForgotPassword(){
  ...
  const handleSubmit = async (e) => {
    e && e.preventDefault();
    if (!email) return setStatus({ ok: false, msg: 'Please enter a valid email.' });
    setLoading(true);
    setStatus(null);
    try{
      ...
      if (res.ok) {
        const msg = data?.message || 'If an account exists, a reset email has been sent.';
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-cyan-50 to-blue-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 text-sm">Don't worry — we'll help you reset it.</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 backdrop-blur-sm border border-white/50">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-600">Enter your email address and we'll send you a secure link to reset your password.</p>
          </div>
            ...
            {status && (
              <div className={`flex items-start space-x-3 p-4 rounded-2xl ${
                ...
                <div>
                  ...
                  {/* Preview URL removed from UI for production */}
                </div>
              </div>
            )}
